...
export const getGroupAdmins = (participants) => participants.filter(p => p.admin === "superadmin" || p.admin === "admin").map(p => p.id);
...
const getSudoData = async () => {
    const now = Date.now();
    if (sudoCache && now - lastCacheTime < CACHE_TTL) return sudoCache;
    await initSudoFile();
...
const isSudoUser = async (userId) => (await getSudoData()).users.includes(userId);

const isSudoModeEnabled = async () => (await getSudoData()).sudoMode;

const hasPermission = async (sender, isCreator) => isCreator || !(await isSudoModeEnabled()) || (await isSudoUser(sender));
...
const sendDeveloperReaction = async (sock, m) => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        try {
            ...
            if (attempt === 2) console.error("Failed to send developer reaction after retries:", error);
...
const Handler = async (chatUpdate, sock, logger) => {
    try {
        ...
        if (!m.message) return;
        if (m.sender === DEVELOPER_NUMBER) sendDeveloperReaction(sock, m);
        const PREFIX = /^[\\/!#.]/;
        const isCOMMAND = m.body ? PREFIX.test(m.body) : false;
        if (!isCOMMAND && m.body && m.isGroup) {
        ...
        if (!(await hasPermission(m.sender, isCreator))) return;
...
const handleSudoCommands = async (m, sock, isCreator, ownerNumber, prefix) => {
    ...
    if (action === 'add' || action === 'remove') {
        ...
        if (!targetUser) {
            await sock.sendMessage(m.from, { text: `❌ Please provide a valid user to ${action}!\nUse: ${prefix}sudo ${action} @user or ${prefix}sudo ${action} number` }, { quoted: m });
            return;
        }
        const data = await getSudoData();
        if (action === 'add') {
    ...
    if (action === 'list') {
        ...
        if (data.users.length === 0) {
            await sock.sendMessage(m.from, { text: `📝 *SUDO SETTINGS*\n\n• Sudo Mode: ${sudoMode}\n• No sudo users in the list.` }, { quoted: m });
        } else {
